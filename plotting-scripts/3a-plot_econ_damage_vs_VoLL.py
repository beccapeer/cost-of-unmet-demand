"""
file name: plot_econ_damage_vs_VoLL.py
    plot economic damage caused by unmet demand as function of value of lost load 
...
# plot results up to reliability = 0
ax.semilogx(np.array(cost_unmet_demand)[0,idx_new], loss_unmet_demand[0,idx_new], 
...
for i in range(1,nfiles):
    ...
    ax.semilogx(np.array(cost_unmet_demand)[i,idx_new], loss_unmet_demand[i,idx_new], 
...
ax.set_ylim(0, np.max(loss_unmet_demand))
ax.set_yticks(np.arange(0,np.max(loss_unmet_demand)+0.02,0.02))
ax.yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
ax.minorticks_off()

# axis labels and legend
xlabel = 'Value of lost load [$/kWh]'
ylabel = 'Economic damage [$/kWh demand]'
...
#fig.savefig(file_path + 'plots/' + '3a-economic damage vs. VoLL' + '.png', 
#            dpi=600, bbox_inches='tight', pad_inches=0.2)
#fig.savefig(file_path + 'plots/' + '3a-economic damage vs. VoLL' + '.svg', 
#            dpi=600, bbox_inches='tight', pad_inches=0.2)
